...
impl<'s, O, P, BS> TopperModule<'s, TimeSlice<O, P>, BS> for TelnetModule {
    type Siblings = ();
    fn handle_message(
        &mut self,
        message: &TopperMessage<TimeSlice<O, P>>,
...
fn handle_connection<BS: Serialize>(
    mut stream: TcpStream,
    receive_lines: &Receiver<String>,
) -> Result<(), Error> {
    ...
    loop {
        ...
        match stream.read(&mut buffer) {
            Ok(size) => {
                if size > 0 {
                    inactive = 0;
                    send_response(&TopperResponse::<BS>::qeb(
